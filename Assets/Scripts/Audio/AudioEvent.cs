...
public class AudioEvent : AudioEventBase
{
    ...
    public override void Play(AudioSource audioSource)
    {
        ...
        audioSource.volume = Random.Range(AudioManager.Instance.GetSFXVolume() /Volume - RandomizeVolume, AudioManager.Instance.GetSFXVolume()/Volume); // get the volume from the audio manager
        audioSource.pitch = Random.Range(Pitch - (RandomizePitch / 2), Pitch + RandomizePitch / 2);
        //audioSource.PlayOneShot(audioSource.clip, 1f);
        //audioSource.PlayOneShot(audioSource.clip, audioSource.volume);
        audioSource.PlayOneShot(audioSource.clip, AudioManager.Instance.GetSFXVolume() /Volume); // get the volume from the audio manager
    }
}